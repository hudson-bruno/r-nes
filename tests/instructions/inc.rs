...
fn test_inc_zero_page() {
    let mut cpu = Cpu::new();
    cpu.memory[0..4].copy_from_slice(&[0xE6, 0x03, 0x00, 0x01]);
...
fn test_inc_zero_page_x() {
    ...
    cpu.memory[0..3].copy_from_slice(&[0xF6, 0xFE, 0x00]);
...
fn test_inc_zero_page_x_overflow() {
    ...
    cpu.memory[0..4].copy_from_slice(&[0xF6, 0xFF, 0x00, 0x01]);
...
fn test_inc_absolute() {
    let mut cpu = Cpu::new();
    cpu.memory[0..3].copy_from_slice(&[0xEE, 0xFF, 0x07]);
...
fn test_inc_absolute_x() {
    let mut cpu = Cpu::new();
    cpu.x_index_register = 0x01;
    cpu.memory[0..3].copy_from_slice(&[0xFE, 0xFE, 0x07]);
    cpu.memory[0x07FF] = 0x01;
